/* PV6 M2.9 — Pastoreo con manejo (PV6)
...
(function(){
  ...
  function ensureCard(){
    ...
    card.innerHTML=`
      <div class="row" style="align-items:center;gap:8px">
        <h3 class="title">Pastoreo con manejo (PV6)</h3><span class="badge">M2.9</span><span id="pv6-m2-note"></span>
  ...
  function ocupadosAFecha(endISO, idx){
    ...
    const pots=allPots();
    for(const p of pots){
      const occ=lastOnOrBefore(idx.occIndex,p,endISO,null);
      if(occ!=null){ if(Number(occ)>0) out.push(p); continue; }
      const ua =lastOnOrBefore(idx.uaIndex ,p,endISO,0);
      if(ua>0) out.push(p);
    }
  ...
  async function init(){
    ...
    console.log("[M2.9] listo — areaHa:", Object.keys(PV6.data?.areaHaByPot||{}).length, "movRows:", rows.length, "esperaOK:", ok);
  }
